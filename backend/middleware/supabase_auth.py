...
def check_auth():
    """Function-call version of require_auth for use in before_request hooks.

    Sets flask.g.user_id and flask.g.email on success.
    Returns None on success, or a flask Response tuple (response, status_code) on failure.
    """
    auth_header = flask.request.headers.get("Authorization", "")
    if not auth_header.startswith("Bearer "):
        return flask.jsonify({"error": "authorization required"}), 401
